...
// Enterprise services implementing the Core Financial Ledger Engine and wallet rules for MASARI
...
class TransactionService {
  ...
  Future<void> finalizePaymentGatewayRecharge(String transactionId, String gatewayTxId) async {
    ...
    // Write trace to immutable double-entry bookkeeping ledger
    final ledgerEntry = LedgerEntry(
      ...
      debit: 0.0,
      credit: tx.amount,
      balanceAfter: tx.amount, // Balance update will be completed by WalletService
...
class WalletService {
  ...
  Future<WalletEntity> payForBooking(String userId, String bookingId, double amount, String bookingTitleAr, String bookingTitleEn) async {
    ...
    final updatedWallet = wallet.copyWith(
      availableBalance: wallet.availableBalance - amount,
      reservedBalance: wallet.reservedBalance + amount, // Temporarily reserved until finalized
      lastUpdatedAt: DateTime.now().toString(),
    );
  ...
  Future<WalletEntity> refundBooking(String userId, String bookingId, double amount, String bookingTitleAr, String bookingTitleEn) async {
    ...
    // Standard refund policy: releases escrow and credits available balance
    final updatedWallet = wallet.copyWith(
      availableBalance: wallet.availableBalance + amount,
  ...
  Future<WalletEntity> transferFunds(String senderUserId, String receiverUserId, double amount) async {
    ...
    // Log sender outflow transaction
    ...
    // Log receiver inflow transaction
